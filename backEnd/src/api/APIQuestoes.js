let questoesHumanas = [
    {
        id: 1,
        areaConhecimento: 'Ciências Humanas e suas Tecnologias',
        triNivel: 'Fácil',
        pontuacaoQuestao: 3,
        textoQuestao: "(ENEM 2023) TEXTO I Como presença consciente no mundo não posso escapar à responsabilidade ética no meu mover-me no mundo. Se sou puro produto da determinação genética ou cultural ou de classe, sou irresponsável pelo que faço no meu mover-me no mundo e, se careço de responsabilidade, não posso falar em ética. FREIRE, P. Pedagogia da autonomia: saberes necessários à prática educativa. São Paulo: Paz e Terra, 1996 // TEXTO II Paulo Freire construiu uma pedagogia da esperança. Na sua concepção, a história não é algo pronto e acabado. As estruturas de opressão e as desigualdades, apesar de serem naturalizadas, são sócio e historicamente construídas. Daí a importância de os educandos tomarem consciência da sua realidade para, assim, transformá-la. DEMARCHI, J. L. Paulo Freire. Disponivel em: https://diplomatique.org.br. Acesso em: 6 out. 2021 (adaptado).",
        enunciado: "Com base no conceito de ética pedagógica presente nos textos, os educandos tornam-se responsáveis pela",
        alternativas: {
            A: "participação sociopolítica.",
            B: "competição econômica local.",
            C: "participação sociopolítica.",
            D: "manutenção do sistema escolar.",
            E: "capacitação de mobilidade individual."
        },
        respostaCorreta: "A"
    },
    {
        id: 2,
        areaConhecimento: 'Ciências Humanas e suas Tecnologias',
        triNivel: 'Fácil',
        pontuacaoQuestao: 3,
        textoQuestao: "(ENEM 2023) TEXTO I Como presença consciente no mundo não posso escapar à responsabilidade ética no meu mover-me no mundo. Se sou puro produto da determinação genética ou cultural ou de classe, sou irresponsável pelo que faço no meu mover-me no mundo e, se careço de responsabilidade, não posso falar em ética. FREIRE, P. Pedagogia da autonomia: saberes necessários à prática educativa. São Paulo: Paz e Terra, 1996 // TEXTO II Paulo Freire construiu uma pedagogia da esperança. Na sua concepção, a história não é algo pronto e acabado. As estruturas de opressão e as desigualdades, apesar de serem naturalizadas, são sócio e historicamente construídas. Daí a importância de os educandos tomarem consciência da sua realidade para, assim, transformá-la. DEMARCHI, J. L. Paulo Freire. Disponivel em: https://diplomatique.org.br. Acesso em: 6 out. 2021 (adaptado).",
        enunciado: "Com base no conceito de ética pedagógica presente nos textos, os educandos tornam-se responsáveis pela",
        alternativas: {
            A: "participação sociopolítica.",
            B: "competição econômica local.",
            C: "participação sociopolítica.",
            D: "manutenção do sistema escolar.",
            E: "capacitação de mobilidade individual."
        },
        respostaCorreta: "A"
    },
    {
        id: 3,
        areaConhecimento: 'Ciências Humanas e suas Tecnologias',
        triNivel: 'Fácil',
        pontuacaoQuestao: 3,
        textoQuestao: "(ENEM 2023) TEXTO I Como presença consciente no mundo não posso escapar à responsabilidade ética no meu mover-me no mundo. Se sou puro produto da determinação genética ou cultural ou de classe, sou irresponsável pelo que faço no meu mover-me no mundo e, se careço de responsabilidade, não posso falar em ética. FREIRE, P. Pedagogia da autonomia: saberes necessários à prática educativa. São Paulo: Paz e Terra, 1996 // TEXTO II Paulo Freire construiu uma pedagogia da esperança. Na sua concepção, a história não é algo pronto e acabado. As estruturas de opressão e as desigualdades, apesar de serem naturalizadas, são sócio e historicamente construídas. Daí a importância de os educandos tomarem consciência da sua realidade para, assim, transformá-la. DEMARCHI, J. L. Paulo Freire. Disponivel em: https://diplomatique.org.br. Acesso em: 6 out. 2021 (adaptado).",
        enunciado: "Com base no conceito de ética pedagógica presente nos textos, os educandos tornam-se responsáveis pela",
        alternativas: {
            A: "participação sociopolítica.",
            B: "competição econômica local.",
            C: "participação sociopolítica.",
            D: "manutenção do sistema escolar.",
            E: "capacitação de mobilidade individual."
        },
        respostaCorreta: "A"
    }
];

let questoesMatematica = [
    {
        id: 1,
        areaConhecimento: 'Ciências Humanas e suas Tecnologias',
        triNivel: 'Fácil',
        pontuacaoQuestao: 3,
        textoQuestao: "(ENEM 2023) TEXTO I Como presença consciente no mundo não posso escapar à responsabilidade ética no meu mover-me no mundo. Se sou puro produto da determinação genética ou cultural ou de classe, sou irresponsável pelo que faço no meu mover-me no mundo e, se careço de responsabilidade, não posso falar em ética. FREIRE, P. Pedagogia da autonomia: saberes necessários à prática educativa. São Paulo: Paz e Terra, 1996 // TEXTO II Paulo Freire construiu uma pedagogia da esperança. Na sua concepção, a história não é algo pronto e acabado. As estruturas de opressão e as desigualdades, apesar de serem naturalizadas, são sócio e historicamente construídas. Daí a importância de os educandos tomarem consciência da sua realidade para, assim, transformá-la. DEMARCHI, J. L. Paulo Freire. Disponivel em: https://diplomatique.org.br. Acesso em: 6 out. 2021 (adaptado).",
        enunciado: "Com base no conceito de ética pedagógica presente nos textos, os educandos tornam-se responsáveis pela",
        alternativas: {
            A: "participação sociopolítica.",
            B: "competição econômica local.",
            C: "participação sociopolítica.",
            D: "manutenção do sistema escolar.",
            E: "capacitação de mobilidade individual."
        },
        respostaCorreta: "A"
    },
]

let questoesLinguagem = [
    {
        id: 1,
        areaConhecimento: 'Ciências Humanas e suas Tecnologias',
        triNivel: 'Fácil',
        pontuacaoQuestao: 3,
        textoQuestao: "(ENEM 2023) TEXTO I Como presença consciente no mundo não posso escapar à responsabilidade ética no meu mover-me no mundo. Se sou puro produto da determinação genética ou cultural ou de classe, sou irresponsável pelo que faço no meu mover-me no mundo e, se careço de responsabilidade, não posso falar em ética. FREIRE, P. Pedagogia da autonomia: saberes necessários à prática educativa. São Paulo: Paz e Terra, 1996 // TEXTO II Paulo Freire construiu uma pedagogia da esperança. Na sua concepção, a história não é algo pronto e acabado. As estruturas de opressão e as desigualdades, apesar de serem naturalizadas, são sócio e historicamente construídas. Daí a importância de os educandos tomarem consciência da sua realidade para, assim, transformá-la. DEMARCHI, J. L. Paulo Freire. Disponivel em: https://diplomatique.org.br. Acesso em: 6 out. 2021 (adaptado).",
        enunciado: "Com base no conceito de ética pedagógica presente nos textos, os educandos tornam-se responsáveis pela",
        alternativas: {
            A: "participação sociopolítica.",
            B: "competição econômica local.",
            C: "participação sociopolítica.",
            D: "manutenção do sistema escolar.",
            E: "capacitação de mobilidade individual."
        },
        respostaCorreta: "A"
    },
]

let questoesNatureza = [
    {
        id: 1,
        areaConhecimento: 'Ciências Humanas e suas Tecnologias',
        triNivel: 'Fácil',
        pontuacaoQuestao: 3,
        textoQuestao: "(ENEM 2023) TEXTO I Como presença consciente no mundo não posso escapar à responsabilidade ética no meu mover-me no mundo. Se sou puro produto da determinação genética ou cultural ou de classe, sou irresponsável pelo que faço no meu mover-me no mundo e, se careço de responsabilidade, não posso falar em ética. FREIRE, P. Pedagogia da autonomia: saberes necessários à prática educativa. São Paulo: Paz e Terra, 1996 // TEXTO II Paulo Freire construiu uma pedagogia da esperança. Na sua concepção, a história não é algo pronto e acabado. As estruturas de opressão e as desigualdades, apesar de serem naturalizadas, são sócio e historicamente construídas. Daí a importância de os educandos tomarem consciência da sua realidade para, assim, transformá-la. DEMARCHI, J. L. Paulo Freire. Disponivel em: https://diplomatique.org.br. Acesso em: 6 out. 2021 (adaptado).",
        enunciado: "Com base no conceito de ética pedagógica presente nos textos, os educandos tornam-se responsáveis pela",
        alternativas: {
            A: "participação sociopolítica.",
            B: "competição econômica local.",
            C: "participação sociopolítica.",
            D: "manutenção do sistema escolar.",
            E: "capacitação de mobilidade individual."
        },
        respostaCorreta: "A"
    },
]

// Função para imprimir a questão
function imprimirQuestao(questao) {
    console.log(`ID: ${questao.id}`);
    console.log(`Texto da Questão: ${questao.textoQuestao}`);
    console.log(`Enunciado: ${questao.enunciado}`);
    console.log("Alternativas:");
    for (let [letra, texto] of Object.entries(questao.alternativas)) {
        console.log(`${letra}: ${texto}`);
    }
    console.log(`Resposta Correta: ${questao.respostaCorreta}`);
}

// Imprime a questão
imprimirQuestao(questoesHumanas[0]);
