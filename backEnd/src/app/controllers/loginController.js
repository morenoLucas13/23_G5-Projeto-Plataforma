...
rotas.post('/cadastrarUser', async (req, res) => {
    ...
    try {
        const { nome, email, senha, nivel_acesso, disciplinasAula, turma, matricula } = req.body
        ...
        if (nivel_acesso === 2) {
            novoUser = await model.cadastrarProfessor(nome, email, senha, nivel_acesso, disciplinasAula)
        } else if (nivel_acesso === 1) {
            novoUser = await model.cadastrarAluno(nome, email, senha, matricula, turma)
        }

        const token = gerarToken(novoUser.id, nivel_acesso)
        ...
        return res.status(500).json({
            sucesso: false,
            mensagem: 'Erro ao cadastrar novo usuário. Tente novamente mais tarde'
        })
    }
